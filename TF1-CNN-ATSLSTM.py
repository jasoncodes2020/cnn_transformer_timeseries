...
def fit_lstm(X,y, batch_size, nb_epoch, neurons):
	...
	model.add(ATSLSTM(4, return_sequences=False))
# 	model.add(ATSLSTM(28, return_sequences=False))
# 	model.add(Dropout(0.0609))
	model.add(Dense(1))
# 	adam = optimizers.Adam(lr=0.05, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=False)
...
def experiment(series5, series6, series7, series18, updates, look_back, neurons, n_epoch, batch_size):
    ...
    def pre_process(img_size=(660, 4)):
    ...
    model.compile(loss='mse',
                  optimizer=keras.optimizers.Adam(lr=0.000012, beta_1=0.9, beta_2=0.99, epsilon=None, decay=0.0,
                                                     amsgrad=False),
                  metrics=['mse'])
    # In[24]:
    ...
    import time
    start = time.clock()
    history1 = model.fit(X_train, y_train, batch_size=32, epochs=300, validation_split=0.2,verbose=1,callbacks=[TensorBoard])
...
run()
with open("./loss/cnn-atslstm-loss.csv", "w", encoding="utf-8") as fout:
